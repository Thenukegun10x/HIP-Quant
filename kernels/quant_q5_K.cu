...
// Serial make_qkx2_quants for nmax=31, rmin=-0.5, rdelta=0.1, nstep=15
__device__ inline float make_qkx2_quants_q5(
    ...
    float iscale = 31.0f / (max_v - min_v);
    ...
    for (int is = 0; is <= 15; ++is) {
        iscale = (-0.5f + 0.1f * (float)is + 31.0f) / (max_v - min_v);
...
void quantize_q5_K_kernel(
    ...
    if (lane == 0) {
        ...
        float min_val;
        float scale_val = make_qkx2_quants_q5(grp, s_x, s_w, s_L, &min_val);
    ...
    if (tid == 0) {
        ...
        block_q5_K *blk = (block_q5_K*)(dst + (row * (n_per_row / QK_K) + sb) * sizeof(block_q5_K));
    ...
    // Pack quants: qs stores low nibbles (same layout as Q4_K), qh stores 5th bit
    block_q5_K *blk = (block_q5_K*)(dst + (row * (n_per_row / QK_K) + sb) * sizeof(block_q5_K));

    // qs: pair L[i] with L[i+32] (chunks of 64)
    if (tid < 128) {
        int chunk = tid / 32;
        int l = tid % 32;
        int lo = s_L[chunk * 64 + l] & 0x0F;
        int hi = s_L[chunk * 64 + l + 32] & 0x0F;
        blk->qs[tid] = (uint8_t)(lo | (hi << 4));
    }

    // qh: single thread to avoid write-write race on same byte (4 chunks map to same 32 bytes)
    if (tid == 0) {
        for (int l = 0; l < 32; ++l) blk->qh[l] = 0;
        for (int c = 0; c < 4; ++c) {
            for (int l = 0; l < 32; ++l) {
                blk->qh[l] |= ((s_L[c * 64 + l] >> 4) & 1) << (c * 2);
                blk->qh[l] |= ((s_L[c * 64 + l + 32] >> 4) & 1) << (c * 2 + 1);
            }
        }
    }
}
