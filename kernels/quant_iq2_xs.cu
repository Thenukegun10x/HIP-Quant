...
__global__ void quantize_iq2_xs_kernel(
    ...
    const int n_sub_blocks = QK_K / 16;
    int row = blockIdx.x;
    int ibl = blockIdx.y;
    int ib = threadIdx.x;
    if (row >= nrows) return;
    ...
    __shared__ float s_scales[QK_K / 16];
    __shared__ uint16_t s_q2[2 * (QK_K / 16)];
    __shared__ uint8_t s_scales_packed[QK_K / 32];

    for (int i = threadIdx.x; i < 2 * n_sub_blocks; i += 256) s_q2[i] = 0;
    __syncthreads();
...

    {
            ...
            if (threadIdx.x == 0) {
                ...
                if (max_scale == 0) {
                    ...
                    for (int j = 0; j < n_sub_blocks; ++j) {
                        ...
                        if (l < 0) l = 0;
                        if (l > 15) l = 15;
                        if (j % 2 == 0) s_scales_packed[j / 2] = (uint8_t)l;
                        else s_scales_packed[j / 2] |= (uint8_t)(l << 4);
                    }
                    for (int i = 0; i < QK_K / 32; ++i) y->scales[i] = s_scales_packed[i];
                    uint8_t * qs = (uint8_t *)y->qs;
                    const uint8_t * q2b = (const uint8_t *)s_q2;
        ...
        for (int k = 0; k < 2; ++k) {
            ...
            s_q2[2 * ib + k] = (uint16_t)(grid_index | ((uint16_t)block_signs[k] << 9));
        }
        local_scale = scale;
    }
