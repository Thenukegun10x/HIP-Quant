...
__global__ void quantize_iq3_xxs_kernel(
    ...
    __shared__ uint32_t s_scales_signs[QK_K / 32];
    __shared__ uint8_t s_q3[3 * (QK_K / 8) + QK_K / 32];

    for (int i = threadIdx.x; i < sizeof(s_q3); i += 256) s_q3[i] = 0;
    __syncthreads();
...

    {
        ...
        if (max_val < GROUP_MAX_EPS_IQ3_XXS) {
            s_scales[ib] = 0;
            s_scales_signs[ib] = 0;
            for (int k = 0; k < 8; ++k) s_q3[8 * ib + k] = 0;
            __syncthreads();
            if (threadIdx.x == 0) {
                float max_scale = 0;
                ...
                if (max_scale == 0) {
                    y->d = 0;
                    for (int i = 0; i < 3 * QK_K / 8; ++i) y->qs[i] = 0;
                }
            }
            return;
        }
        ...
        for (int k = 0; k < 8; ++k) {
            ...
            s_q3[8 * ib + k] = (uint8_t)grid_index;
        }

        s_scales_signs[ib] = (uint32_t)block_signs[0]
    ...
    if (threadIdx.x == 0) {
        ...
        if (max_scale == 0) {
            ...
            for (int j = 0; j < n_sub_blocks; ++j) {
                ...
                if (l < 0) l = 0;
                if (l > 15) l = 15;
                s_scales_signs[j] |= ((uint32_t)l << 28);
            }
            uint8_t * qs = y->qs;
            for (int i = 0; i < QK_K / 4; ++i) qs[i] = s_q3[i];
            for (int i = 0; i < QK_K / 32; ++i) {
                ((uint32_t *)(qs + QK_K / 4))[i] = s_scales_signs[i];
            }
        }
    }
}
